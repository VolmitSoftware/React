...
void main() {
  group('RoleBadge', () {
    testServer('renders Admin for admin role', (ServerTester tester) async {
    ...
    testServer('renders Operator for operator role', (
      ServerTester tester,
    ) async {
      tester.pumpComponent(
        _wrap(
    ...
    testServer('renders Viewer for viewer role', (ServerTester tester) async {
      tester.pumpComponent(
        _wrap(
          RoleBadge(
            role: const RoleInfo(role: 'viewer', scopes: <String>['read']),
          ),
        ),
      );
      final DocumentResponse res = await tester.request('/');
    ...
    testServer('renders nothing when role is null', (
      ServerTester tester,
    ) async {
      tester.pumpComponent(_wrap(RoleBadge(role: null)));
      final DocumentResponse res = await tester.request('/');
      expect(res.statusCode, equals(200));
      expect(
        res.body.contains('Admin') ||
