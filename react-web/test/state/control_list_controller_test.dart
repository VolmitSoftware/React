...
import 'package:react_web/state/control_list_controller.dart';

ControlItem _item(
  String id, {
  bool enabled = false,
  List<Knob> knobs = const <Knob>[],
}) => ControlItem(
  id: id,
  name: id,
  category: 'test',
...
Knob _knob(String key, Object? value) =>
    Knob(key: key, label: key, type: KnobType.intType, value: value);
...
class _FakeControlClient implements IControlClient {
  ...
  Future<ControlItem> Function(String id, Map<String, Object?> knobs)?
  onSetFeatureConfig;
  Future<ControlItem> Function(String id, Map<String, Object?> knobs)?
  onSetTweakConfig;
...
void main() {
  ...
  group('ControlListController.toggle optimism', () {
    test('applies optimistic update before server responds', () async {
      ...
      client.onSetFeatureEnabled = (String id, bool enabled) =>
          completer.future;
      ...
      expect(
        ...
        isTrue,
        reason:
            'optimistic update must be visible synchronously before server responds',
      );
    ...
    test('replaces item with server result after future completes', () async {
      ...
      client.onSetFeatureEnabled = (String id, bool enabled) async =>
          serverItem;
  ...
  group('ControlListController.toggle rollback', () {
    test(
      'restores previous value and calls onError on server failure',
      () async {
        final _FakeControlClient client = _FakeControlClient();
        client.featuresResult = <ControlItem>[_item('a', enabled: false)];
        client.onSetFeatureEnabled = (String id, bool enabled) async =>
  ...
  group('ControlListController.setKnob optimism', () {
    test('applies optimistic knob value before server responds', () async {
      ...
      final Completer<ControlItem> completer = Completer<ControlItem>();
      client.onSetFeatureConfig = (String id, Map<String, Object?> knobs) =>
          completer.future;
      ...
      final ControlItem optimisticItem = controller.items.firstWhere(
        (ControlItem i) => i.id == 'a',
      );
      expect(
      ...
      final ControlItem serverItem = _item(
        'a',
        knobs: <Knob>[_knob('speed', 99)],
      );
  ...
  group('ControlListController.setAll', () {
    test(
      'toggles only items whose enabled differs and calls setFeatureEnabled correct count',
      () async {
        final _FakeControlClient client = _FakeControlClient();
        client.featuresResult = <ControlItem>[
        ...
        expect(
          ...
          equals(2),
          reason:
              'setFeatureEnabled must be called only for items that changed',
        );
      },
    );
  });
