...
import 'package:react_web/state/role_scope.dart';
...
class _ForbiddenClient implements IRoleClient {
  @override
  Future<RoleInfo> whoami() async => throw const ReactForbidden('not allowed');
}
...
void main() {
  group('RoleScope', () {
    ...
    test('updateShouldNotify returns true when scopes change', () {
      final RoleScope s1 = RoleScope(
        role: const RoleInfo(role: 'admin', scopes: <String>['read']),
        child: Component.fragment(const <Widget>[]),
      );
      final RoleScope s2 = RoleScope(
        role: const RoleInfo(
          role: 'admin',
          scopes: <String>['read', 'console:execute'],
        ),
        child: Component.fragment(const <Widget>[]),
      );
      expect(s1.updateShouldNotify(s2), isTrue);
    ...
    test(
      'updateShouldNotify returns true when going from null to non-null',
      () {
        final RoleScope s1 = RoleScope(
          role: null,
          child: Component.fragment(const <Widget>[]),
  ...
  group('RoleObserver', () {
    testServer('probe renders admin role after whoami resolves', (
      ServerTester tester,
    ) async {
      tester.pumpComponent(
        _wrap(RoleObserver(client: _AdminClient(), child: const _Probe())),
      );
    ...
    testServer('probe renders nothing when client throws ReactForbidden', (
      ServerTester tester,
    ) async {
      tester.pumpComponent(
        _wrap(RoleObserver(client: _ForbiddenClient(), child: const _Probe())),
      );
      final DocumentResponse res = await tester.request('/');
      expect(res.statusCode, equals(200));
      expect(
    ...
    testServer('probe renders nothing when client is null', (
      ServerTester tester,
    ) async {
      tester.pumpComponent(
        _wrap(RoleObserver(client: null, child: const _Probe())),
      );
      final DocumentResponse res = await tester.request('/');
      expect(res.statusCode, equals(200));
      expect(
