...
import 'package:react_web/state/fleet_export.dart';
...
void main() {
  group('FleetManager.importReplace', () {
    test('replaces existing servers with incoming creds', () {
      ...
      expect(
        fm.servers.map((ServerCredential c) => c.id),
  ...
  group('FleetManager.remove persists', () {
    test(
      'after remove a fresh FleetManager does not contain the removed id',
      () {
        final InMemoryFleetStorage storage = InMemoryFleetStorage();
        final FleetManager fm = _seeded(
          storage: storage,
  ...
  group('parseFleetImportJson validation', () {
    test('rejects invalid JSON', () {
      final FleetParseResult result = parseFleetImportJson(
        'not valid json {{{{',
      );
