  ...
  group('HeatmapScope', () {
    testServer('probe renders marker when client is non-null', (
      ...
      tester.pumpComponent(
        _wrap(
          HeatmapScope(client: _FakeHeatmapClient(), child: const _Probe()),
        ),
      );
      ...
      expect(
        ...
        isTrue,
        reason:
            'marker must appear when HeatmapScope carries a non-null client',
      );
    });
    ...
    testServer('probe does not render marker when client is null', (
      ...
      tester.pumpComponent(
        _wrap(HeatmapScope(client: null, child: const _Probe())),
      );
      final DocumentResponse res = await tester.request('/');
      expect(res.statusCode, equals(200));
      expect(
        ...
        isFalse,
        reason:
            'marker must not appear when HeatmapScope carries a null client',
      );
    });
  });
}
