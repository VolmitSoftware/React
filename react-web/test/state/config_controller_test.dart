...
import 'package:react_web/state/config_controller.dart';
    ...
    ConfigSection(
      ...
      nodes: <Knob>[
        for (final String k in keys)
          Knob(key: k, label: k, type: KnobType.intType, value: 1),
      ],
    ),
  ],
);
...
void main() {
  ...
  group('ConfigController.apply', () {
    test(
      'calls applyConfig with pending map and replaces tree on success',
      () async {
        final _FakeConfigClient client = _FakeConfigClient();
        ...
        expect(
          client.capturedApplyConfigArgs,
  ...
  group('ConfigController.applyPreset', () {
    test(
      'calls applyPreset with name, replaces tree, clears pending',
      () async {
        final _FakeConfigClient client = _FakeConfigClient();
