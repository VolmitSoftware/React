...
void main() {
  ...
  group('FleetLiveModel — snapshot delivery', () {
    test(
      'onChange fires and servers[A].snapshot/lastSeen update after snapshot arrives',
      () async {
        final StreamController<ServerSnapshot> snapA =
            StreamController<ServerSnapshot>.broadcast();
        final StreamController<ConnState> stateA =
        ...
        final FleetServerLive serverA = model.servers.firstWhere(
          (FleetServerLive s) => s.id == 'a',
        );
        expect(serverA.snapshot, isNotNull);
        expect(serverA.lastSeen, isNotNull);
        final FleetServerLive serverB = model.servers.firstWhere(
          (FleetServerLive s) => s.id == 'b',
        );
  ...
  group('FleetLiveModel — update()', () {
    test(
      'update() removing a source reduces servers list and old stream events are ignored',
      () async {
        final StreamController<ServerSnapshot> snapA =
            StreamController<ServerSnapshot>.broadcast();
        final StreamController<ConnState> stateA =
    ...
    test(
      'update() adding a new source preserves existing server snapshots',
      () async {
        final StreamController<ServerSnapshot> snapB =
            StreamController<ServerSnapshot>.broadcast();
        final StreamController<ConnState> stateB =
        ...
        final FleetServerLive serverBBefore = model.servers.firstWhere(
          (FleetServerLive s) => s.id == 'b',
        );
        ...
        expect(model.servers.length, equals(2));
        final FleetServerLive serverBAfter = model.servers.firstWhere(
          (FleetServerLive s) => s.id == 'b',
        );
  ...
  group('FleetLiveModel — dispose()', () {
    test(
      'dispose() prevents further onChange calls from stream events',
      () async {
        final StreamController<ServerSnapshot> snapA =
            StreamController<ServerSnapshot>.broadcast();
        final StreamController<ConnState> stateA =
