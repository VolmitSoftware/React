...
void main() {
  group('FleetAlert.key', () {
    test('key is serverId/metricId', () {
      final FleetAlert alert = _makeAlert(
        serverId: 's1',
  ...
  group('FleetAlert equality', () {
    test(
      'two alerts with same serverId+metricId but different value are equal',
      () {
        final FleetAlert a = _makeAlert(
          serverId: 's1',
          metricId: 'ticks-per-second',
          value: 15.0,
        );
        final FleetAlert b = _makeAlert(
          serverId: 's1',
          metricId: 'ticks-per-second',
          value: 9.0,
        );
    ...
    test(
      'two alerts with same serverId+metricId collapse in a Set to length 1',
      () {
        final FleetAlert a = _makeAlert(
          serverId: 's1',
          metricId: 'ticks-per-second',
  ...
  group('alertSeverityRank', () {
    test('critical has highest rank', () {
      expect(
        alertSeverityRank(AlertSeverity.critical),
        greaterThan(alertSeverityRank(AlertSeverity.warning)),
      );
    });
    ...
    test('warning is higher than info', () {
      expect(
        ...
        greaterThan(alertSeverityRank(AlertSeverity.info)),
      );
    });
