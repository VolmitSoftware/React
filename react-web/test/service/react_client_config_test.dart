...
void main() {
  ...
  Map<String, dynamic> configBody(double gcValue) => <String, dynamic>{
    'data': <String, dynamic>{
      'sections': <dynamic>[
        <String, dynamic>{
          'name': 'Memory',
  ...
  group('ReactClient.applyConfig()', () {
    test(
      'PUTs /api/v1/config with counter header and flat changes map',
      () async {
        http.Request? captured;
        final MockClient mock = MockClient((http.Request req) async {
          captured = req;
          return http.Response(
            jsonEncode(configBody(75.0)),
            200,
            headers: <String, String>{'content-type': 'application/json'},
          );
        });
        ...
        final ReactClient client = ReactClient(cred, client: mock);
        final ConfigTree result = await client.applyConfig(<String, Object?>{
          'gc.threshold': 75.0,
        });
  ...
  group('ReactClient.applyPreset()', () {
    test(
      'POSTs /api/v1/config/preset/balanced with counter and returns tree',
      () async {
        http.Request? captured;
        final Map<String, dynamic> responseBody = <String, dynamic>{
          'data': <String, dynamic>{'sections': <dynamic>[]},
        };
