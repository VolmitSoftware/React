...
void main() {
  group('ActionsScreen route wiring', () {
    ...
    test('buildReactorRoutes includes server actions route', () {
      ...
      expect(
        hasActions,
        isTrue,
        reason: 'per-server Actions screen requires /server/:id/actions route',
      );
    });
  });

  group('ActionsScreen sidebar', () {
    testServer('ReactorShell with a server renders Actions sidebar item', (
      ServerTester tester,
    ) async {
      tester.pumpComponent(
        _wrapView(
          ReactorShell(
            currentPath: '/server/srv1/actions',
      ...
      expect(
        ...
        isTrue,
        reason: 'Actions sidebar item must appear in per-server sidebar group',
      );
    });
  });
  ...
  group('ActionsConsoleView render', () {
    testServer('renders all action names', (ServerTester tester) async {
      tester.pumpComponent(
        _wrapView(
          ActionsConsoleView(
    ...
    testServer('renders Destructive chip for destructive action only', (
      ServerTester tester,
    ) async {
      tester.pumpComponent(
        _wrapView(
    ...
    testServer('renders Execute button per action card', (
      ServerTester tester,
    ) async {
      tester.pumpComponent(
        _wrapView(
    ...
    testServer('renders Recent Executions heading', (
      ServerTester tester,
    ) async {
      tester.pumpComponent(
        _wrapView(
