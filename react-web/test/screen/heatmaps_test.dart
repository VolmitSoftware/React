...
import 'package:react_web/state/heatmap_scope.dart';
...
void main() {
  group('HeatmapsScreen', () {
    testServer('renders Spatial Metrics section card heading', (
      ServerTester tester,
    ) async {
    ...
    testServer('renders Entity Pressure stat tile label', (
      ServerTester tester,
    ) async {
      tester.pumpComponent(_wrap(const HeatmapsScreen()));
      final DocumentResponse res = await tester.request('/');
    ...
    testServer('renders entity-pressure-heatmap value 0.7', (
      ServerTester tester,
    ) async {
      tester.pumpComponent(_wrap(const HeatmapsScreen()));
      final DocumentResponse res = await tester.request('/');
      ...
      expect(
        ...
        isTrue,
        reason:
            'entity-pressure-heatmap value 0.7 must appear in rendered HTML',
      );
    });

    testServer('does not render the old chunk-grid placeholder', (
      ServerTester tester,
    ) async {
      tester.pumpComponent(_wrap(const HeatmapsScreen()));
      final DocumentResponse res = await tester.request('/');
