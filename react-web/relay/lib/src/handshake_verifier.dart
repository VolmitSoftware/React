...
class HandshakeVerifier {
  ...
  Future<bool> verify({
    ...
    try {
      ...
      if (nonceBytes == null || nonceBytes.length != 32) return false;

      final List<int>? spkiBytes = decodeServerPubKey(pubKeyBase64Url);
      if (spkiBytes == null ||
          spkiBytes.length != kEd25519SpkiPrefix.length + 32) {
        return false;
      }
      for (int index = 0; index < kEd25519SpkiPrefix.length; index++) {
        if (spkiBytes[index] != kEd25519SpkiPrefix[index]) return false;
      }

      final List<int> rawKey = spkiBytes.sublist(kEd25519SpkiPrefix.length);
      ...
      final Signature signature = Signature(sigBytes, publicKey: publicKey);
