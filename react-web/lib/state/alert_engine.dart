...
class AlertEngine {
      ...
      if (tps.value < thresholds.tpsCrit) {
        results.add(
          _make(
            ...
            title: reactorText(ReactorText.alertLowTps),
    ...
    if (mspt != null && mspt.value > thresholds.msptWarn) {
      results.add(
        _make(
          ...
          sampler: mspt,
          severity: AlertSeverity.warning,
          title: reactorText(ReactorText.alertHighMspt),
    ...
    if (incident != null && incident.value > thresholds.incidentScoreWarn) {
      results.add(
        _make(
          ...
          sampler: incident,
          severity: AlertSeverity.warning,
          title: reactorText(ReactorText.alertElevatedIncidentScore),
    ...
    if (gc != null && gc.value > thresholds.gcPercentWarn) {
      results.add(
        _make(
          ...
          sampler: gc,
          severity: AlertSeverity.warning,
          title: reactorText(ReactorText.alertHighGcTime),
    ...
    if (ping != null && ping.value > thresholds.pingP95Warn) {
      results.add(
        _make(
          ...
          sampler: ping,
          severity: AlertSeverity.warning,
          title: reactorText(ReactorText.alertHighPingP95),
    ...
    if (mem != null && mem.value > thresholds.memoryPressureWarn) {
      results.add(
        _make(
          ...
          sampler: mem,
          severity: AlertSeverity.warning,
          title: reactorText(ReactorText.alertMemoryPressure),
  ...
  static FleetAlert _make({
    ...
    final String suffix = sampler.suffix.isEmpty ? '' : ' ${sampler.suffix}';
    final String comparison = ltThreshold ? '(< $threshold)' : '(> $threshold)';
    return FleetAlert(
      serverId: serverId,
      serverName: serverName,
      metricId: sampler.id,
