...
/**
 * Edict class represents the main processing and controlling entity of the
 * Edict command handling system. It encapsulates the primary functionality of
 * the system and holds all necessary components such as parserPackages,
 * contextResolverPackages, endpoints, contextResolvers and parsers.
 * <br><br>
 * This class is equipped with initialization capabilities to load default
 * parsers and context resolvers and to record detailed status information.
 *
 * @see EdictParser
 * @see EdictEndpoint
...
public class Edict implements CommandExecutor, TabCompleter {
  /**
   * List of parser packages for the Edict system. The parsers in these packages
   * are used to convert input strings into their corresponding objects. By
   * default, this list is initialized as an empty ArrayList.
   */
  @Singular("parserPackage")
  private final List<String> parserPackages;

  /**
   * List of context resolver packages for the Edict system. The context
   * resolvers in these packages are used to derive contextual data from input
   * commands. By default, this list is initialized as an empty ArrayList.
   */
  @Singular("contextResolverPackage")
  private final List<String> contextResolverPackages;
  /**
   * Set of context resolvers for the Edict system. These resolvers are used to
   * derive contextual data from input commands.
   */
  @Singular
  private final Set<EdictContextResolver<?>> contextResolvers = defaultContextResolvers();
  /**
   * Set of parsers for the Edict system. These parsers are used to convert
   * input strings into their corresponding objects.
   */
  @Singular
  ...
  /**
   * This method splits a string into a list of arguments. It divides the input
   * string on spaces and removes any empty strings from the result.
   *
   * @param args The string to be split into arguments. This should be a
   *             space-separated string.
   * @return A list of strings, each of which is a non-empty argument from the
   * input string.
   */
   ...
   * <br><br>
   * If the sub-package name does not start with a dot (.), the method appends
   * it at the beginning to form the correct structure.
   *
  ...
  /**
   * This method creates a set of default parsers by dynamically instantiating
   * all implementations of the EdictParser interface found in the
   * "art.arcane.edict.content.parser" package. These parsers are typically used
   * to parse input strings into EdictValue objects. This method uses reflection
   * to discover and instantiate the parsers, hence any failure in the
   * instantiation process is silently ignored.
   *
   * @return A set of EdictParser objects, each one an instance of a different
   * implementation of the EdictParser interface found in the
   * "art.arcane.edict.content.parser" package.
   */
  ...
  /**
   * This method creates a set of default context resolvers by dynamically
   * instantiating all implementations of the EdictParser interface found in the
   * "art.arcane.edict.content.context" package. These resolvers are typically
   * used to resolve what objects are related to the sender. This method uses
   * reflection to discover and instantiate the parsers, hence any failure in
   * the instantiation process is silently ignored.
   *
   * @return A set of EdictContextResolver objects, each one an instance of a
   * different implementation of the EdictContextResolver interface found in the
   * "art.arcane.edict.content.context" package.
   */
   ...
   * the instantiation process is silently ignored.
   *
   * @param and A list of additional packages to search for more context
   *            resolvers.
   * @return A set of EdictContextResolver objects, each one an instance of a
   * different implementation of the EdictContextResolver interface found in the
   * "art.arcane.edict.content.context" package.
  ...
  /**
   * Enhances the provided list of arguments by applying a series of
   * transformations, specifically: processing quotes and enhancing mappings.
   *
  ...
  /**
   * Enhances the provided list of arguments by processing quotes and optionally
   * trimming whitespace.
   *
   * @param args The list of arguments to be enhanced.
   * @return A new list of enhanced arguments.
  ...
  /**
   * Enhances the provided list of arguments by processing quotes, with the
  ...
  /**
   * Calculates the Levenshtein distance between two strings, which is a measure
   * of the minimum number of single-character edits (insertions, deletions, or
  ...
  /**
   * Calculates the cost of substituting one character for another in a string.
   * The cost is zero if the characters are the same, and one otherwise.
   *
  ...
  /**
   * Initializes the Edict system by loading the default parsers and context
   * resolvers. The detailed status of the system, including the number of
   * endpoints, parsers, and context resolvers, is also reported.
   */
  ...
  /**
   * This method processes an EdictRequest and generates a Stream of
   * EdictResponses. Each response is based on one of the available endpoints.
   * The request is mapped to each endpoint, and if the mapping is successful, a
   * response is generated. The responses are streamed, meaning they are
   * generated and processed one at a time, which can be more efficient for
   * large numbers of endpoints.
   *
   * @param request The EdictRequest object containing the inputs to be mapped.
   * @return A Stream of EdictResponse objects. Each response corresponds to a
   * different endpoint and contains information about how the inputs from the
   * request map to the fields in the endpoint.
   */
  ...
  /**
   * Executes a given command with a specific sender. This method creates an
   * {@link EdictContext} for the execution and invokes the executor for the
   * appropriate endpoint.
   *
   * @param sender  the originator of the command.
   * @param command the command to be executed.
   * @return an {@link Optional} that contains an {@link EdictResponse} if the
   * command execution was successful; otherwise, if there's no corresponding
   * endpoint for the command, returns an empty Optional.
   */
  ...
  /**
   * Responds to an {@link EdictRequest}. This method finds the most suitable
   * response to the request, which is determined by the minimum score offset.
   *
   * @param request the {@link EdictRequest} to respond to.
   * @return an {@link Optional} that contains the most suitable
   * {@link EdictResponse} if found; otherwise, if no suitable response is
  ...
  /**
   * This method processes an EdictRequest and generates an EdictResponse based
   * on the provided EdictEndpoint. It maps the inputs of the request to the
   * fields in the endpoint, then generates an EdictFieldResponse for each
   * field. The EdictFieldResponses are generated based on the confidence level
   * of the match between the input and the field. The input is removed from the
   * input list once a match is found. If no input matches a field, an
   * EdictFieldResponse with INVALID confidence is added.
   *
   * @param request  The EdictRequest object containing the inputs to be
   *                 mapped.
   * @param endpoint The EdictEndpoint object containing the fields to which the
   *                 inputs are to be mapped.
   * @return An EdictResponse object containing the original request, the
   * endpoint, a match score, and a list of EdictFieldResponses. Each
   * EdictFieldResponse corresponds to a field in the endpoint, and contains
   * information about the matching input and the confidence level of the
   * match.
   */
  ...
  /**
   * This method generates an EdictRequest object from a string of arguments.
   * The string is split into individual arguments based on spaces.
   *
   * @param args A string of space-separated arguments to be included in the
   *             request.
   * @return An EdictRequest object with the arguments split from the input
   * string.
   */
   ...
   * @return An EdictRequest object with the arguments taken from the input
   * array.
   */
  public EdictRequest request(String[] args) {
    return EdictRequest.of(this, args);
  ...
  /**
   * Parses the given input string into an EdictInput. If the string contains an
   * "=" character, it is split into a key and a value, and a
   * MappedEdictInputHolder is returned. If not, a PositionalEdictInputHolder is
   * returned, which holds the original string and the positional index.
   *
   * @param var             The input string to be parsed into an EdictInput. If
   *                        it contains "=", it is treated as a key-value pair.
   * @param positionalIndex The position of the input string in the original
   *                        sequence of arguments. This is used when creating a
   *                        PositionalEdictInputHolder.
   * @param realIndex       The index of the input in the overall sequence,
   *                        considering all inputs, not just the positional
   *                        ones. This is also used when creating a
   *                        PositionalEdictInputHolder.
   * @return A new EdictInput, which can be either a MappedEdictInputHolder if
   * the input string contains "=", or a PositionalEdictInputHolder otherwise.
   */
  ...
  /**
   * Parses the given input string into a list of EdictValue objects. Each
   * object in the list is generated by one of the parsers available in the
   * parsers list. This means the input string is parsed multiple times, once
   * for each parser.
   *
   * @param value The input string to be parsed into EdictValue objects.
   * @return A list of EdictValue objects. Each object represents a possible
   * parsing of the input string, based on one of the available parsers.
   */
   ...
   *
   * @param superPackage The package the class must at least start with to
   *                     check
   */
  ...
  /**
   * Register a cursed component (class) as an endpoint using annotations checks
   * all static methods for @Command annotations
   *
  ...
  /**
   * Define the command header to match this method. This is the command that
   * will be used to invoke the method. Do not specify arguments. If it's a root
   * command simply type the command. You can use a slash, but it's not
   * required.
   */
   ...
   * not be matched by the matcher very well.
   * <br><br>
   * For parameters, the base name is captured by the method parameter name, but
   * you can add aliases to it in the same way
   */
  ...
  /**
   * Indicate that this command can only be run by players. By default commands
   * can be run by both players and the console.
   */
  @Target(ElementType.METHOD)
  @Retention(RetentionPolicy.RUNTIME)
  ...
  /**
   * Indicate that this command can only be run by the console. By default
   * commands can be run by both players and the console.
   */
  @Target(ElementType.METHOD)
  @Retention(RetentionPolicy.RUNTIME)
  ...
  /**
   * Denote that if this parameter is not specified in the command, a context
   * grab will fill this parameter. For example:
   *
   * @Contextual Player p, ... This would grab the player that executed the
   * command if it was a player.
   */
  ...
  /**
   * Denote that this parameter must be non-null and the confidence cannot be
   * INVALID. If you specify this on a method, it will enforce that all
  ...
  /**
   * Provide a default value to be parsed into this parameter if it's not
   * specified in the command or if the confidence is INVALID
   */
  @Target(ElementType.PARAMETER)
  @Retention(RetentionPolicy.RUNTIME)
  ...
  /**
   * Define a description for the command. This will be displayed in the help
   * menu. You can also use this on parameters
   */
  @Target({ElementType.METHOD, ElementType.PARAMETER})
  @Retention(RetentionPolicy.RUNTIME)
