...
/**
 * The EdictField class represents a field within an Edict command in the Edict
 * command handling system. Each field has a list of names it can be identified
 * by, a type which defines the kind of data it can hold, and a default value
 * which can be used when no other value is provided.
 * <br><br>
 * This class is typically used in conjunction with the {@link EdictEndpoint}
 * class to define a command's expected inputs.
 */
@AllArgsConstructor
@Builder
@Data
public class EdictField {
  /**
   * The names by which this field can be identified. The field can be referred
   * to by any of these names.
   */
  @Singular
  ...
  /**
   * Whether or not this field is required. If it is, the command will fail if
   * no input is provided. Defaults to false
   */
  @Builder.Default
