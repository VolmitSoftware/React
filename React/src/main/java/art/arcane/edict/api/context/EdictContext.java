 ...
 * It holds important information such as the Edict object, the endpoint, the
 * request, and the response of the operation. It also provides a mechanism for
 * resolving context-based dependencies through the use of `resolve` method.
 * Thread-local instances of the `EdictContext` can be managed through the `put`
 * and `get` static methods, providing a way to ensure thread safety.
 *
 * @see Edict
 * @see EdictEndpoint
 * @see EdictRequest
...
public class EdictContext {
  ...
  /**
   * A map for storing resolved objects by their class types. It's used in the
   * `resolve` method to cache and reuse resolved objects.
   */
  @Builder.Default
   ...
   * @return The previous `EdictContext` associated with the current thread, or
   ...
   *
   * @return The `EdictContext` associated with the current thread, or null if
   * there is none.
   */
   ...
   * @return The resolved object or null if no resolver could be found for the
   * given class.
   */
