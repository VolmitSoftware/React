...
/**
 * Interface that should be implemented by parsers intended to support enum
 * types.
 * <br><br>
 * The implemented parser can process input strings and map them to appropriate
 * enum constants. The parse method is designed to take into account different
 * possible formatting of the input strings (such as lower case, upper case,
 * dashes or underscores), and tries to find the enum constant that best matches
 * the input string.
 * <br><br>
 * Furthermore, the implemented parser is able to suggest possible inputs based
 * on the enum constants, and also to declare whether the input should strictly
 * be one of these suggested inputs.
 *
...
public interface EnumeratedParser<T extends Enum<T>> extends EdictParser<T>, Suggestive {
  /**
   * Calculates the similarity between the enum constant name and the input
   * string. The similarity is determined by the Levenshtein distance between
   * the two strings.
   *
   ...
   * @return an EdictValue instance which encapsulates the parsed enum constant
  ...
  /**
   * Returns whether the input should strictly match one of the suggested
   * inputs.
   *
   * @return true if the input should match one of the suggestions, false
   * otherwise
   */
