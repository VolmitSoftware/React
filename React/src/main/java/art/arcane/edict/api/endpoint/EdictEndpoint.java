...
/**
 * The EdictEndpoint class represents a command endpoint in an Edict command
 * handling system. It encapsulates the command itself, a method to execute the
 * command, and a list of fields which can be populated with input values when
 * the command is executed.
 * <br><br>
 * The class also provides methods for getting the command path and the name of
 * the command, derived from the command string itself.
 */
@AllArgsConstructor
@Builder
@Data
public class EdictEndpoint {
  /**
   * The command to be executed. Format just the header not the parameters such
   * as "/plugin subcommand"
   */
  ...
  /**
   * Additional command references as aliases. These still require the full
   * command format such as "/plugin altcommand"
   */
  @Singular
  private List<String> aliases;

  /**
   * Who is allowed to execute this command. Players or Consoles. Defaults to
   * ANY
   */
  @Builder.Default
  ...
  /**
   * The list of fields which can be populated with input values when the
   * command is executed.
   */
  @Singular
  ...
  /**
   * Returns the name of the command, which is the last segment of the command
   * path.
   *
  ...
  /**
   * Returns the path of the command, where each segment is an element in the
   * list.
   *
