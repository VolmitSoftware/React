...
abstract class FeatureIrisChunkSharePieBase extends ReactFeature implements Listener, ReactRenderer {
    ...
    public final void render() {
        ...
        drawPie(centerX, centerY, 29, 11, slices, total);
        drawLegend(69, 17, slices);
    ...
    private void drawLegend(int startX, int startY, List<Slice> slices) {
        ...
        for (Slice slice : slices) {
            ...
            set(startX - 2, y - 1, 56, 12, PANEL);
            set(startX - 2, y - 1, 56, 1, PANEL_BORDER);
            set(startX - 2, y + 10, 56, 1, PANEL_BORDER);
            set(startX, y + 1, 6, 6, slice.color());
            String percent = Form.f(slice.ratio() * 100D, 1) + "%";
            String count = compact(slice.count());
            text(startX + 9, y, trimLabel(slice.label(), 10));
            text(startX + 9, y + 8, percent + "  " + count);
            index++;
        }
    }
