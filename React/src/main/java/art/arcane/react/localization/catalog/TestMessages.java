...
public final class TestMessages {
  ...
  public static final TextKey MAP_CLIP_BLANK = TextKey.of("test.map.clip_blank", "The clip probe drew nothing inside its region.");
  public static final TextKey MAP_CLIP_FAIL = TextKey.of("test.map.clip_fail", "Drawing escaped the active clip rect.");
  public static final TextKey MAP_CLIP_PASS = TextKey.of("test.map.clip_pass", "Lines, text and fills stayed inside the pushed clip rect.");
  public static final TextKey MAP_CLIP_RESTORE_FAIL = TextKey.of("test.map.clip_restore_fail", "popClip did not restore the previous clip rect.");
  public static final TextKey MAP_FLUSH_FIDELITY_FAIL = TextKey.of("test.map.flush_fidelity_fail", "The buffered flush did not reproduce the composed surface:");
  public static final TextKey MAP_FLUSH_FIDELITY_PASS = TextKey.of("test.map.flush_fidelity_pass", "Every flushed pixel matches the composed surface at 1x1, 2x2 and 3x3.");
  public static final TextKey MAP_FLUSH_STABILITY_FAIL = TextKey.of("test.map.flush_stability_fail", "Repainting identical content still wrote to the canvas:");
  public static final TextKey MAP_FLUSH_STABILITY_PASS = TextKey.of("test.map.flush_stability_pass", "Repainting identical content writes zero canvas pixels.");
  public static final TextKey MAP_FRAME_WALL_DISABLED = TextKey.of("test.map.frame_wall_disabled", "Megamap tiling is disabled; adjacent frames never form a wall.");
  public static final TextKey MAP_FRAME_WALL_FAIL = TextKey.of("test.map.frame_wall_fail", "Adjacent item frames never settled into a two-tile megamap wall:");
  public static final TextKey MAP_FRAME_WALL_NO_SPACE = TextKey.of("test.map.frame_wall_no_space", "No clear air column near spawn to build a two-frame map wall.");
  public static final TextKey MAP_FRAME_WALL_NO_WORLDS = TextKey.of("test.map.frame_wall_no_worlds", "No worlds are loaded; cannot build a live item-frame wall.");
  public static final TextKey MAP_FRAME_WALL_PASS = TextKey.of("test.map.frame_wall_pass", "Cloned React maps in adjacent frames split to distinct ids and solved into a two-tile wall.");
  public static final TextKey MAP_FRAME_WALL_PROBE_FAILED = TextKey.of("test.map.frame_wall_probe_failed", "Probing the live item-frame wall failed:");
  public static final TextKey MAP_FRAME_WALL_SETUP_FAILED = TextKey.of("test.map.frame_wall_setup_failed", "Could not build a live item-frame wall:");
  public static final TextKey MAP_MEGAMAP_CAPACITY_FAIL = TextKey.of("test.map.megamap_capacity_fail", "Megamap capability derivation is wrong:");
  public static final TextKey MAP_MEGAMAP_CAPACITY_PASS = TextKey.of("test.map.megamap_capacity_pass", "Megamap capabilities derive columns, rows and detail from the grid.");
  public static final TextKey MAP_MEGAMAP_DEFECTS_FAIL = TextKey.of("test.map.megamap_defects_fail", "Broken map walls were not diagnosed:");
  public static final TextKey MAP_MEGAMAP_DEFECTS_PASS = TextKey.of("test.map.megamap_defects_pass", "Ragged, rotated, duplicated and mixed map walls all report a reason.");
  public static final TextKey MAP_MEGAMAP_EXPANSION_FAIL = TextKey.of("test.map.megamap_expansion_fail", "Renderers failed on an expanded wall:");
  public static final TextKey MAP_MEGAMAP_EXPANSION_PASS = TextKey.of("test.map.megamap_expansion_pass", "Every renderer fills every frame of a 1x1, 2x1, 1x2, 1x3, 2x2, 3x1, 3x3 and 5x3 wall.");
  public static final TextKey MAP_MEGAMAP_VIEWPORT_FAIL = TextKey.of("test.map.megamap_viewport_fail", "Megamap viewport geometry is wrong:");
  public static final TextKey MAP_MEGAMAP_VIEWPORT_PASS = TextKey.of("test.map.megamap_viewport_pass", "Adaptive walls get one logical canvas and magnified walls stay uniform and letterboxed.");
  public static final TextKey MAP_PALETTE_FAIL = TextKey.of("test.map.palette_fail", "Theme colors collapse on the map palette:");
  public static final TextKey MAP_PALETTE_PASS = TextKey.of("test.map.palette_pass", "Every theme color is an exact, distinct map palette entry.");
  public static final TextKey MAP_RENDERER_CONTAINMENT_FAIL = TextKey.of("test.map.renderer_containment_fail", "Renderers drew outside their clip rect:");
  public static final TextKey MAP_RENDERER_CONTAINMENT_PASS = TextKey.of("test.map.renderer_containment_pass", "Every renderer stayed inside the pushed clip rect.");
  public static final TextKey MAP_RENDERER_CONTAINMENT_SKIP = TextKey.of("test.map.renderer_containment_skip", "No renderer could be exercised headlessly.");
  public static final TextKey MAP_TEXT_BOUNDS_FAIL = TextKey.of("test.map.text_bounds_fail", "Fitted text exceeded its width limit.");
  public static final TextKey MAP_TEXT_BOUNDS_PASS = TextKey.of("test.map.text_bounds_pass", "Fitted text never exceeds its limit at any ui scale.");
  ...
  public static final TextKey MONITOR_NO_INTEGRATIONS = TextKey.of("test.monitor.no_integrations", "No Iris, Adapt, Wormholes, Gloss, HiddenOre, or BileTools integration samplers present.");
  ...
  public static void addTo(MessageCatalog.Builder builder) {
    ...
    builder.add(MAP_CLIP_BLANK);
    builder.add(MAP_CLIP_FAIL);
    builder.add(MAP_CLIP_PASS);
    builder.add(MAP_CLIP_RESTORE_FAIL);
    builder.add(MAP_FLUSH_FIDELITY_FAIL);
    builder.add(MAP_FLUSH_FIDELITY_PASS);
    builder.add(MAP_FLUSH_STABILITY_FAIL);
    builder.add(MAP_FLUSH_STABILITY_PASS);
    builder.add(MAP_FRAME_WALL_DISABLED);
    builder.add(MAP_FRAME_WALL_FAIL);
    builder.add(MAP_FRAME_WALL_NO_SPACE);
    builder.add(MAP_FRAME_WALL_NO_WORLDS);
    builder.add(MAP_FRAME_WALL_PASS);
    builder.add(MAP_FRAME_WALL_PROBE_FAILED);
    builder.add(MAP_FRAME_WALL_SETUP_FAILED);
    builder.add(MAP_MEGAMAP_CAPACITY_FAIL);
    builder.add(MAP_MEGAMAP_CAPACITY_PASS);
    builder.add(MAP_MEGAMAP_DEFECTS_FAIL);
    builder.add(MAP_MEGAMAP_DEFECTS_PASS);
    builder.add(MAP_MEGAMAP_EXPANSION_FAIL);
    builder.add(MAP_MEGAMAP_EXPANSION_PASS);
    builder.add(MAP_MEGAMAP_VIEWPORT_FAIL);
    builder.add(MAP_MEGAMAP_VIEWPORT_PASS);
    builder.add(MAP_PALETTE_FAIL);
    builder.add(MAP_PALETTE_PASS);
    builder.add(MAP_RENDERER_CONTAINMENT_FAIL);
    builder.add(MAP_RENDERER_CONTAINMENT_PASS);
    builder.add(MAP_RENDERER_CONTAINMENT_SKIP);
    builder.add(MAP_TEXT_BOUNDS_FAIL);
    builder.add(MAP_TEXT_BOUNDS_PASS);
