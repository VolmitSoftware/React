...
import 'package:reactor/screen/environment.dart';
...
EnvironmentInfo _fakeInfo() => EnvironmentInfo(
      sections: LinkedHashMap<String, Map<String, Object?>>.from(<String, Map<String, Object?>>{
...
void main() {
  ...
  group('ReactorShell sidebar', () {
    testServer(
      'renders Environment sidebar item for a connected server',
      (ServerTester tester) async {
        tester.pumpComponent(
          _wrapView(ReactorShell(
            servers: const <ServerEntry>[
              ServerEntry(id: 'srv1', name: 'Test Server', state: ConnState.live),
            ],
          )),
        );
  ...
  group('EnvironmentView', () {
    testServer(
      'renders Apple M3 value from cpu section',
      (ServerTester tester) async {
    ...
    testServer(
      'renders cores value 8 from cpu section',
      (ServerTester tester) async {
        tester.pumpComponent(_wrapView(EnvironmentView(info: _fakeInfo())));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
    ...
    testServer(
      'renders usedMb value 18000 from memory section',
      (ServerTester tester) async {
        tester.pumpComponent(_wrapView(EnvironmentView(info: _fakeInfo())));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
    ...
    testServer(
      'renders Purpur value from server section',
      (ServerTester tester) async {
        tester.pumpComponent(_wrapView(EnvironmentView(info: _fakeInfo())));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
    ...
    testServer(
      'renders Memory section heading',
      (ServerTester tester) async {
        tester.pumpComponent(_wrapView(EnvironmentView(info: _fakeInfo())));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
    ...
    testServer(
      'renders Server section heading',
      (ServerTester tester) async {
        tester.pumpComponent(_wrapView(EnvironmentView(info: _fakeInfo())));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
    ...
    testServer(
      ...
      (ServerTester tester) async {
        ...
        expect(
          ...
          isTrue,
          reason: 'live-connection note must appear when OperateScope client is null',
        );
      },
    );
  });
