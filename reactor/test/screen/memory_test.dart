...
import 'package:reactor/screen/memory.dart';
...
void main() {
  group('MemoryScreen', () {
    testServer(
      'renders used-heap tile showing memory-used value 3500.0',
    ...
    testServer(
      'renders gc-time-percent gauge label and value',
      (ServerTester tester) async {
        tester.pumpComponent(_wrap(const MemoryScreen()));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
        ...
        expect(
          ...
          isTrue,
          reason:
              'gc-time-percent value 1.8 must appear as gauge center numeric',
        );
      },
    );

    testServer(
      'renders Memory Pressure section heading',
      (ServerTester tester) async {
        tester.pumpComponent(_wrap(const MemoryScreen()));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
    ...
    testServer(
      'renders gc-pause-p95 as line chart section',
      (ServerTester tester) async {
        tester.pumpComponent(_wrap(const MemoryScreen()));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
    ...
    testServer(
      'renders Memory Garbage stat tile label and value',
      (ServerTester tester) async {
        tester.pumpComponent(_wrap(const MemoryScreen()));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
