...
import 'package:reactor/screen/chunks.dart';
...
void main() {
  group('ChunksScreen', () {
    testServer(
      'renders Chunks stat tile label',
    ...
    testServer(
      'renders Persistence section card heading',
      (ServerTester tester) async {
        tester.pumpComponent(_wrap(const ChunksScreen()));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
    ...
    testServer(
      'renders World Save stat tile label',
      (ServerTester tester) async {
        tester.pumpComponent(_wrap(const ChunksScreen()));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
    ...
    testServer(
      'renders Chunk Load/Gen Time section card heading',
      (ServerTester tester) async {
        tester.pumpComponent(_wrap(const ChunksScreen()));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
