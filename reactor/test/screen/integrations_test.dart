...
import 'package:reactor/screen/integrations.dart';
...
void main() {
  group('IntegrationsScreen — all integrations present', () {
    testServer(
      'renders Adapt section when adapt-* samplers are present',
      (ServerTester tester) async {
        ...
        expect(
          ...
          isTrue,
          reason: 'Adapt section heading must appear when adapt-* samplers are present',
        );
      },
    );

    testServer(
      'renders Iris section when iris-* samplers are present',
      (ServerTester tester) async {
        tester.pumpComponent(
          _wrapWith(const IntegrationsScreen(), _fakeSnapshotWithIntegrations()),
        );
        ...
        expect(
          ...
          isTrue,
          reason: 'Iris section heading must appear when iris-* samplers are present',
        );
      },
    );
    ...
    testServer(
      'renders at least one sampler value from the snapshot',
      (ServerTester tester) async {
        tester.pumpComponent(
          _wrapWith(const IntegrationsScreen(), _fakeSnapshotWithIntegrations()),
        );
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
        final bool hasValue = res.body.contains('14.0') ||
            res.body.contains('8.4') ||
            res.body.contains('5.6');
        expect(
          hasValue,
          isTrue,
          reason: 'at least one integration sampler value must appear in rendered HTML',
        );
      },
    );
  });
    ...
    testServer(
      ...
      (ServerTester tester) async {
        ...
        expect(
          ...
          isTrue,
          reason:
              'empty-state description must match spec exactly',
        );
      },
    );

    testServer(
      'does NOT render Adapt section when no adapt-* samplers present',
      (ServerTester tester) async {
        tester.pumpComponent(
          _wrapWith(const IntegrationsScreen(), _fakeSnapshotNoIntegrations()),
        );
  ...
  group('IntegrationsScreen route wiring', () {
    ...
    test('buildReactorRoutes includes server integrations route', () {
      ...
      expect(
        hasIntegrations,
        isTrue,
        reason: 'per-server Integrations tab requires /server/:id/integrations route',
      );
    });
  });
}
