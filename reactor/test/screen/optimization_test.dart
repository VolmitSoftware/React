...
void main() {
  group('OptimizationScreen route wiring', () {
    ...
    test('buildReactorRoutes includes server optimization route', () {
      ...
      final bool hasOptimization = routes.any(
        (RouteBase r) =>
            r is Route && r.path == kRouteServerOptimization,
      );
      expect(
        hasOptimization,
        isTrue,
  ...
  group('OptimizationScreen sidebar', () {
    testServer(
      'ReactorShell with a server renders Optimization sidebar item',
  ...
  group('OptimizationGridView render', () {
    testServer(
      ...
      (ServerTester tester) async {
        tester.pumpComponent(_wrapView(OptimizationGridView(
          items: _fakeItems,
    ...
    testServer(
      'renders feature names Mob Stacking and Fast Leaf Decay',
      (ServerTester tester) async {
        tester.pumpComponent(_wrapView(OptimizationGridView(
          items: _fakeItems,
          total: _fakeItems.length,
    ...
    testServer(
      'renders enabled summary 1 / 2 enabled',
      (ServerTester tester) async {
        tester.pumpComponent(_wrapView(OptimizationGridView(
          items: _fakeItems,
          total: _fakeItems.length,
    ...
    testServer(
      'renders Enable all and Disable all buttons',
      (ServerTester tester) async {
        tester.pumpComponent(_wrapView(OptimizationGridView(
          items: _fakeItems,
          total: _fakeItems.length,
    ...
    testServer(
      'renders Configure action for each feature card',
      (ServerTester tester) async {
        tester.pumpComponent(_wrapView(OptimizationGridView(
          items: _fakeItems,
          total: _fakeItems.length,
