...
import 'package:reactor/screen/incidents.dart';
    ...
    testServer(
      ...
      (ServerTester tester) async {
        ...
        expect(
          ...
          isTrue,
          reason: 'Incident Timeline section heading must appear in rendered HTML',
        );
      },
    );

    testServer(
      'renders Backlog section heading',
      (ServerTester tester) async {
        tester.pumpComponent(_wrap(const IncidentsScreen()));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
  ...
  group('IncidentsScreen route', () {
    ...
    test('buildReactorRoutes includes kRouteServerIncidents route', () {
      ...
      expect(
        hasIncidents,
        isTrue,
        reason: 'per-server Incidents tab requires route at kRouteServerIncidents',
      );
    });
  });
}
