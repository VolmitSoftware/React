...
import 'package:reactor/screen/config_editor.dart';
...
void main() {
  group('ConfigEditorScreen route wiring', () {
    ...
    test('buildReactorRoutes includes server config route', () {
      ...
      expect(
        ...
        isTrue,
        reason:
            'Config Editor screen requires /server/:id/config route',
      );
    });
  });
  ...
  group('ConfigEditorScreen sidebar', () {
    testServer(
      'ReactorShell with a server renders Config Editor sidebar item',
  ...
  group('ConfigEditorView render', () {
    testServer(
      'renders both section names Performance and Entity',
      (ServerTester tester) async {
        tester.pumpComponent(_wrapView(ConfigEditorView(
          tree: _fakeTree(),
    ...
    testServer(
      'renders node labels Tick Rate and Max Entities',
      (ServerTester tester) async {
        tester.pumpComponent(_wrapView(ConfigEditorView(
          tree: _fakeTree(),
          pending: const <String, Object?>{},
    ...
    testServer(
      'renders Off Light Balanced High preset buttons',
      (ServerTester tester) async {
        tester.pumpComponent(_wrapView(ConfigEditorView(
          tree: _fakeTree(),
          pending: const <String, Object?>{},
        ...
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
        expect(res.body.contains('Off'), isTrue,
            reason: 'Off preset button must appear');
        expect(res.body.contains('Light'), isTrue,
            reason: 'Light preset button must appear');
        expect(res.body.contains('Balanced'), isTrue,
            reason: 'Balanced preset button must appear');
        expect(res.body.contains('High'), isTrue,
            reason: 'High preset button must appear');
      },
    );

    testServer(
      'renders Apply Changes button',
      (ServerTester tester) async {
        tester.pumpComponent(_wrapView(ConfigEditorView(
          tree: _fakeTree(),
          pending: const <String, Object?>{},
