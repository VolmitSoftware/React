...
import 'package:reactor/screen/performance.dart';
...
void main() {
  group('PerformanceScreen', () {
    testServer(
      'renders tick-time section heading',
    ...
    testServer(
      'renders top-world-mspt tile label and value',
      (ServerTester tester) async {
        tester.pumpComponent(_wrap(const PerformanceScreen()));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
    ...
    testServer(
      'renders tick-spike-rate as sparkline section',
      (ServerTester tester) async {
        tester.pumpComponent(_wrap(const PerformanceScreen()));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
        expect(
          ...
          isTrue,
          reason:
              'Tick Spike Rate section heading must appear in rendered HTML',
        );
        // A sparkline must not render the current value as a StatTile big
        // number. tick-spike-rate=0.05 → StatTile formats to "0.1"; sparkline
        // chart does not render this text.
        expect(
          res.body.contains('0.1'),
          isFalse,
          reason:
              'tick-spike-rate must render as sparkline chart, not StatTile displaying "0.1"',
        );
      },
    );

    testServer(
      'renders top-chunk-cost tile label and value',
      (ServerTester tester) async {
        tester.pumpComponent(_wrap(const PerformanceScreen()));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
