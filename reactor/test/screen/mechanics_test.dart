...
import 'package:reactor/screen/mechanics.dart';
...
ServerSnapshot _fakeSnapshot() {
  final List<SamplerSample> samples = <SamplerSample>[
    ...
    _sample('redstone-tick-time', 1.2, suffix: 'ms', max: 50.0),
    _sample('hopper', 80.0, max: 10000.0),
    _sample('hopper-tick-time', 0.8, suffix: 'ms', max: 50.0),
    _sample('hopper-chain-coalescing', 12.0, max: 1000.0),
    _sample('physics', 15.0, max: 5000.0),
    _sample('physics-tick-time', 0.5, suffix: 'ms', max: 50.0),
    _sample('fluid', 25.0, max: 5000.0),
    _sample('fluid-tick-time', 0.3, suffix: 'ms', max: 50.0),
...
void main() {
  group('MechanicsScreen', () {
    testServer(
      'renders Redstone section heading',
    ...
    testServer(
      'renders Hoppers section heading',
      (ServerTester tester) async {
        tester.pumpComponent(_wrap(const MechanicsScreen()));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
    ...
    testServer(
      'renders Physics &amp; Fluids section heading',
      (ServerTester tester) async {
        tester.pumpComponent(_wrap(const MechanicsScreen()));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
        expect(
          ...
          isTrue,
          reason:
              'Physics & Fluids section heading must appear in rendered HTML',
        );
      },
    );

    testServer(
      'renders Optimizations section heading',
      (ServerTester tester) async {
        tester.pumpComponent(_wrap(const MechanicsScreen()));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
    ...
    testServer(
      'renders at least one sampler value (redstone 42.0)',
      (ServerTester tester) async {
        tester.pumpComponent(_wrap(const MechanicsScreen()));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
