...
import 'package:reactor/screen/tweaks.dart';
...
void main() {
  ...
  group('TweaksScreen sidebar', () {
    testServer(
      'ReactorShell with a server renders Tweaks sidebar item',
  ...
  group('TweaksListView render', () {
    testServer(
      'renders tweak names Fast Fluids and Fast Snow',
      (ServerTester tester) async {
        tester.pumpComponent(_wrapView(TweaksListView(
          items: _fakeTweaks,
          onToggle: null,
          onKnobChanged: null,
        )));
    ...
    testServer(
      'renders knob label Rate in accordion customContent',
      (ServerTester tester) async {
        tester.pumpComponent(_wrapView(TweaksListView(
          items: _fakeTweaks,
          onToggle: null,
    ...
    testServer(
      'renders Configure (1) accordion title for tweaks with knobs',
      (ServerTester tester) async {
        tester.pumpComponent(_wrapView(TweaksListView(
          items: _fakeTweaks,
          onToggle: null,
        ...
        expect(
          ...
          isTrue,
          reason: 'accordion title Configure (1) must appear for tweaks with 1 knob',
        );
      },
    );
  });
    ...
    testServer(
      ...
      (ServerTester tester) async {
        ...
        expect(
          ...
          isTrue,
          reason:
              'connection note must appear when ControlScope has no client',
        );
      },
    );
  });
