...
import 'package:reactor/screen/incident_center.dart';
...
void main() {
  group('IncidentCenterScreen route wiring', () {
    test('kRouteServerIncidentCenter constant is correct', () {
      expect(
        kRouteServerIncidentCenter,
        equals('/server/:id/incident-center'),
      );
    });
    ...
    test('buildReactorRoutes includes server incident-center route', () {
      ...
      final bool hasRoute = routes.any(
        (RouteBase r) =>
            r is Route && r.path == kRouteServerIncidentCenter,
      );
      expect(
        hasRoute,
        isTrue,
  ...
  group('IncidentCenterView render', () {
    ...
    testServer(
      'renders spike at t0 timeline entry',
      (ServerTester tester) async {
        tester.pumpComponent(
          _wrapView(const IncidentCenterView(status: _fakeStatus)),
        );
    ...
    testServer(
      'renders Contributing Factors section heading',
      (ServerTester tester) async {
        tester.pumpComponent(
          _wrapView(const IncidentCenterView(status: _fakeStatus)),
        );
    ...
    testServer(
      'renders Incident Timeline section heading',
      (ServerTester tester) async {
        tester.pumpComponent(
          _wrapView(const IncidentCenterView(status: _fakeStatus)),
        );
