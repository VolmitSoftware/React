...
import 'package:reactor/screen/entities.dart';
...
void main() {
  group('EntitiesScreen', () {
    ...
    testServer(
      'renders Entities stat tile label',
      (ServerTester tester) async {
        tester.pumpComponent(_wrap(const EntitiesScreen()));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
    ...
    testServer(
      'renders AI Active stat tile label',
      (ServerTester tester) async {
        tester.pumpComponent(_wrap(const EntitiesScreen()));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
    ...
    testServer(
      'renders Player Ping section card heading',
      (ServerTester tester) async {
        tester.pumpComponent(_wrap(const EntitiesScreen()));
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
