...
import 'package:reactor/service/react_socket.dart';
...
void main() {
  ...
  group('ConnectionManager with socket — live on first WS frame', () {
    test('transitions connecting -> live on first injected WS frame', () async {
      ...
      final _FakeMetricsClient client =
  ...
  group('ConnectionManager with socket — snapshot delivery', () {
    test('emitted WS frame appears on snapshots stream with incremented seq',
        () async {
      final _FakeMetricsSocket socket = _FakeMetricsSocket();
      final _FakeMetricsClient client =
          _FakeMetricsClient(<Object>[_makeSnapshot()]);
  ...
  group('ConnectionManager with socket — HTTP fallback on WS close', () {
    test(
        'closing WS stream with no prior frames stays in connecting and '
    ...
    test(
        'closing WS stream AFTER prior frames transitions to degraded then '
    ...
    test('WS error transitions to degraded and HTTP fallback recovers live',
        () async {
      final _FakeMetricsSocket socket = _FakeMetricsSocket();
      // Emit a frame first so _wsHadFrames = true, then error.
      final _FakeMetricsClient client =
          _FakeMetricsClient(<Object>[_makeSnapshot(value: 77.0)]);
    ...
    test('WS error with no prior frames stays in connecting and recovers live',
        () async {
      final _FakeMetricsSocket socket = _FakeMetricsSocket();
      final _FakeMetricsClient client =
          _FakeMetricsClient(<Object>[_makeSnapshot(value: 55.0)]);
  ...
  group('ConnectionManager with socketFactory — reconnect', () {
    test(
        'after WS drop, socketFactory is called to reconnect and live is '
