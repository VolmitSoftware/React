...
void main() {
  ...
  group('ConnectionManager — success path', () {
    ...
    test('emits snapshot to stream on success', () async {
      final _FakeMetricsClient client =
          _FakeMetricsClient(<Object>[_makeSnapshot(cpuValue: 42.0)]);
      final ConnectionManager manager =
          ConnectionManager(client, pollInterval: Duration.zero);
  ...
  group('ConnectionManager — error path', () {
    ...
    test('does not throw from poll errors — stream remains open', () async {
      ...
      manager.snapshots.listen(
        (_) {},
        onError: (_) => streamErrored = true,
      );
