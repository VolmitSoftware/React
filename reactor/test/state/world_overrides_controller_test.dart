...
import 'package:reactor/state/world_overrides_controller.dart';

WorldSettings _world(String name, {double budgetMs = 50.0, double panicMs = 80.0, double releaseMs = 40.0}) =>
    WorldSettings(
      name: name,
...
class _FakeControlClient implements IControlClient {
  ...
  Future<WorldSettings> Function(String name,
      {double? budgetMs, double? panicMs, double? releaseMs})? onSetWorld;
  ...
  Future<WorldSettings> setWorld(String name,
      {double? budgetMs, double? panicMs, double? releaseMs}) {
    if (onSetWorld != null) {
      return onSetWorld!(name, budgetMs: budgetMs, panicMs: panicMs, releaseMs: releaseMs);
    }
    throw UnimplementedError();
  }
}
...
void main() {
  group('WorldOverridesController.load', () {
    test('populates worlds and clears loading on success', () async {
      ...
      client.worldsResult = <WorldSettings>[_world('overworld'), _world('nether')];
      int notifyCount = 0;
  ...
  group('WorldOverridesController.setBudget optimism', () {
    test('applies optimistic budgetMs before server responds', () async {
      ...
      final _FakeControlClient client = _FakeControlClient();
      client.worldsResult = <WorldSettings>[_world('overworld', budgetMs: 50.0)];
      client.onSetWorld = (String name, {double? budgetMs, double? panicMs, double? releaseMs}) =>
          completer.future;

      final WorldOverridesController controller = WorldOverridesController(client);
      await controller.load();

      final Future<void> pending = controller.setBudget('overworld', budgetMs: 75.0);

      expect(
        controller.worlds.firstWhere((WorldSettings w) => w.name == 'overworld').budgetMs,
        equals(75.0),
    ...
    test('replaces world with server result on success', () async {
      ...
      client.worldsResult = <WorldSettings>[_world('overworld', budgetMs: 50.0)];
      final WorldSettings serverResult = _world('overworld', budgetMs: 75.0);
      client.onSetWorld = (String name, {double? budgetMs, double? panicMs, double? releaseMs}) async =>
          serverResult;
      ...
      expect(
        controller.worlds.firstWhere((WorldSettings w) => w.name == 'overworld'),
        same(serverResult),
      );
    });
  });
  ...
  group('WorldOverridesController.setBudget rollback', () {
    test('restores previous budgetMs and calls onError on server failure', () async {
      final _FakeControlClient client = _FakeControlClient();
      client.worldsResult = <WorldSettings>[_world('overworld', budgetMs: 50.0)];
      client.onSetWorld = (String name, {double? budgetMs, double? panicMs, double? releaseMs}) async =>
          throw const ReactConflict();
  ...
  group('WorldOverridesController.setBudget partial fields', () {
    test('only updates provided fields optimistically', () async {
      ...
      client.onSetWorld = (String name, {double? budgetMs, double? panicMs, double? releaseMs}) async =>
          _world(name, budgetMs: budgetMs ?? 50.0, panicMs: panicMs ?? 80.0, releaseMs: releaseMs ?? 40.0);

      final WorldOverridesController controller = WorldOverridesController(client);
      await controller.load();
      await controller.setBudget('overworld', panicMs: 90.0);

      final WorldSettings result =
          controller.worlds.firstWhere((WorldSettings w) => w.name == 'overworld');
