...
void main() {
  group('FleetManager relay transport', () {
    ...
    test(
      '(2) relay-only credential (empty host) adds successfully when '
      'relayClientFactory identity returns pinned fingerprint',
      () async {
    ...
    test(
      '(3) direct-only credential (relayUrl null) behaves as before; '
      'relayClientFactory is never called',
      () async {
    ...
    test(
      '(4) relayClientFactory null + relayUrl set + usable host uses direct '
      'leg alone and add() succeeds',
      () async {
        final FleetManager fm = FleetManager(
          storage: InMemoryFleetStorage(),
