  ...
  group('FleetRollup.compute', () {
    test('two live servers tps 20 players 10+5 no penalties', () {
      final List<FleetServerLive> servers = <FleetServerLive>[
        _liveServer('a', 'Alpha', samplers: <String, double>{
          'ticks-per-second': 20.0,
          'players': 10.0,
        }),
        _liveServer('b', 'Beta', samplers: <String, double>{
          'ticks-per-second': 20.0,
          'players': 5.0,
        }),
      ];
    ...
    test('offline server scores 0 health offline counted as 0 in composite', () {
      final List<FleetServerLive> servers = <FleetServerLive>[
        _liveServer('a', 'Alpha', samplers: <String, double>{
          'ticks-per-second': 20.0,
        }),
      ...
      expect(r.needsAttention.any((FleetServerHealth s) => s.id == 'b'), isTrue);
    });
    ...
    test('alerts grouped by severity per-server alertCount', () {
      ...
      expect(r.needsAttention.any((FleetServerHealth s) => s.id == 'a'), isTrue);
    });

    test('empty servers list produces zero compositeHealth meanTps and empty needsAttention', () {
      final FleetRollup r = FleetRollup.compute(
