...
import 'package:reactor/state/alert_engine.dart';
...
void main() {
  group('AlertEngine.computeForServer', () {
    ...
    test('TPS 9.0 produces exactly one critical TPS alert with threshold 10', () {
      final ServerSnapshot snapshot =
          _snapshotFrom(<String, double>{'ticks-per-second': 9.0});
      final List<FleetAlert> alerts = AlertEngine.computeForServer(
        serverId: 's1',
        serverName: 'Server 1',
        snapshot: snapshot,
    ...
    test('TPS 15.0 produces one warning TPS alert with threshold 18', () {
      final ServerSnapshot snapshot =
          _snapshotFrom(<String, double>{'ticks-per-second': 15.0});
      final List<FleetAlert> alerts = AlertEngine.computeForServer(
        serverId: 's1',
        serverName: 'Server 1',
        snapshot: snapshot,
    ...
    test('TPS 19.9 produces no TPS alert', () {
      final ServerSnapshot snapshot =
          _snapshotFrom(<String, double>{'ticks-per-second': 19.9});
      final List<FleetAlert> alerts = AlertEngine.computeForServer(
        serverId: 's1',
        serverName: 'Server 1',
        snapshot: snapshot,
      ...
      expect(alerts.where((FleetAlert a) => a.metricId == 'ticks-per-second'),
          isEmpty);
    });

    test('tick-time 60 produces warning High MSPT', () {
      final ServerSnapshot snapshot =
          _snapshotFrom(<String, double>{'tick-time': 60.0});
      final List<FleetAlert> alerts = AlertEngine.computeForServer(
        serverId: 's1',
        serverName: 'Server 1',
        snapshot: snapshot,
    ...
    test('tick-time 40 produces no MSPT alert', () {
      final ServerSnapshot snapshot =
          _snapshotFrom(<String, double>{'tick-time': 40.0});
      final List<FleetAlert> alerts = AlertEngine.computeForServer(
        serverId: 's1',
        serverName: 'Server 1',
        snapshot: snapshot,
    ...
    test('incident-score 70 produces warning', () {
      final ServerSnapshot snapshot =
          _snapshotFrom(<String, double>{'incident-score': 70.0});
      final List<FleetAlert> alerts = AlertEngine.computeForServer(
        serverId: 's1',
        serverName: 'Server 1',
        snapshot: snapshot,
    ...
    test('incident-score 30 produces no alert', () {
      final ServerSnapshot snapshot =
          _snapshotFrom(<String, double>{'incident-score': 30.0});
      final List<FleetAlert> alerts = AlertEngine.computeForServer(
        serverId: 's1',
        serverName: 'Server 1',
        snapshot: snapshot,
    ...
    test('gc-time-percent 20 produces warning', () {
      final ServerSnapshot snapshot =
          _snapshotFrom(<String, double>{'gc-time-percent': 20.0});
      final List<FleetAlert> alerts = AlertEngine.computeForServer(
        serverId: 's1',
        serverName: 'Server 1',
        snapshot: snapshot,
    ...
    test('gc-time-percent 5 produces no alert', () {
      final ServerSnapshot snapshot =
          _snapshotFrom(<String, double>{'gc-time-percent': 5.0});
      final List<FleetAlert> alerts = AlertEngine.computeForServer(
        serverId: 's1',
        serverName: 'Server 1',
        snapshot: snapshot,
    ...
    test('player-ping-p95 250 produces warning', () {
      final ServerSnapshot snapshot =
          _snapshotFrom(<String, double>{'player-ping-p95': 250.0});
      final List<FleetAlert> alerts = AlertEngine.computeForServer(
        serverId: 's1',
        serverName: 'Server 1',
        snapshot: snapshot,
    ...
    test('player-ping-p95 100 produces no alert', () {
      final ServerSnapshot snapshot =
          _snapshotFrom(<String, double>{'player-ping-p95': 100.0});
      final List<FleetAlert> alerts = AlertEngine.computeForServer(
        serverId: 's1',
        serverName: 'Server 1',
        snapshot: snapshot,
    ...
    test('memory-pressure 95 produces warning', () {
      final ServerSnapshot snapshot =
          _snapshotFrom(<String, double>{'memory-pressure': 95.0});
      final List<FleetAlert> alerts = AlertEngine.computeForServer(
        serverId: 's1',
        serverName: 'Server 1',
        snapshot: snapshot,
    ...
    test('memory-pressure 50 produces no alert', () {
      final ServerSnapshot snapshot =
          _snapshotFrom(<String, double>{'memory-pressure': 50.0});
      final List<FleetAlert> alerts = AlertEngine.computeForServer(
        serverId: 's1',
        serverName: 'Server 1',
        snapshot: snapshot,
  ...
  group('AlertEngine.computeFleet', () {
    test('orders critical alerts before warnings across two servers', () {
      ...
      final int firstWarningIndex = alerts
          .indexWhere((FleetAlert a) => a.severity == AlertSeverity.warning);
      final int lastCriticalIndex = alerts.lastIndexWhere(
          (FleetAlert a) => a.severity == AlertSeverity.critical);
      expect(lastCriticalIndex < firstWarningIndex, isTrue,
