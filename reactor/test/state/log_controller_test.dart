...
import 'package:reactor/state/log_controller.dart';
...
void main() {
  group('LogController.load', () {
    test('seeds buffer from client.logs and visible matches seeded list',
        () async {
    ...
    test('notifies onChange at least once on success', () async {
      final _FakeLogClient client =
          _FakeLogClient(result: <String>['[INFO] hello']);
      int count = 0;
      final LogController ctrl =
          LogController(client, onChange: () => count++);
    ...
    test('calls onError when client throws', () async {
      ...
      Object? caught;
      final LogController ctrl =
          LogController(client, onError: (Object e) => caught = e);
    ...
    test('replaces buffer on second load call', () async {
      final _FakeLogClient client =
          _FakeLogClient(result: <String>['first load']);
      final LogController ctrl = LogController(client);
      await ctrl.load();
  ...
  group('LogController.start + socket', () {
    ...
    test('paused: line added during pause is NOT appended', () async {
      final _FakeLogClient client =
          _FakeLogClient(result: <String>['[INFO] seed']);
      final _FakeLogSocket socket = _FakeLogSocket();
      final LogController ctrl = LogController(client, socket: socket);
      await ctrl.load();
      ctrl.start();
  ...
  group('LogController.clear', () {
    test('empties visible buffer', () async {
      final _FakeLogClient client =
          _FakeLogClient(result: <String>['[INFO] a', '[ERROR] b']);
      final LogController ctrl = LogController(client);
      await ctrl.load();
