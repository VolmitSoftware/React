...
import 'package:reactor/widget/gauge.dart';
...
void main() {
  group('StatTile.formatValue', () {
    ...
    test('uses the human display, not the raw value', () {
      final SamplerSample sample =
          _fakeSample(value: 1339115552.0, suffix: 'GB', display: '1.3');
  ...
  group('Gauge.statusFor', () {
    test('value below warn threshold returns success', () {
      expect(
        Gauge.statusFor(30.0, (70.0, 90.0)),
        equals(GaugeStatus.success),
      );
    });

    test('value at warn threshold returns warning', () {
      expect(
        Gauge.statusFor(70.0, (70.0, 90.0)),
        equals(GaugeStatus.warning),
      );
    });

    test('value between warn and error thresholds returns warning', () {
      expect(
        Gauge.statusFor(80.0, (70.0, 90.0)),
        equals(GaugeStatus.warning),
      );
    });
    ...
    test('value at error threshold returns error', () {
      expect(
        Gauge.statusFor(90.0, (70.0, 90.0)),
        equals(GaugeStatus.error),
      );
    });

    test('value above error threshold returns error', () {
      expect(
        Gauge.statusFor(95.0, (70.0, 90.0)),
        equals(GaugeStatus.error),
      );
    });
  });
  ...
  group('StatTile rendering', () {
    testServer(
      'renders label text when sample is provided',
    ...
    testServer(
      'renders "--" when sample is null',
      (ServerTester tester) async {
        tester.pumpComponent(
          _wrap(StatTile(label: 'Memory', sample: null)),
        );
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
        expect(
    ...
    testServer(
      ...
      (ServerTester tester) async {
        tester.pumpComponent(
          _wrap(StatTile(
            label: 'Memory Used',
            sample: _fakeSample(value: 1339115552.0, suffix: 'GB', display: '1.3'),
          )),
        );
        final DocumentResponse res = await tester.request('/');
        expect(res.statusCode, equals(200));
  ...
  group('Gauge rendering', () {
    ...
    testServer(
      'renders the provided display string as the center value',
      (ServerTester tester) async {
        tester.pumpComponent(
          _wrap(Gauge(
            label: 'TPS',
