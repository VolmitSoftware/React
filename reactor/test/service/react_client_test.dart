...
void main() {
  ...
  group('ReactClient.history()', () {
    test('GETs /metrics/{id}/history and decodes the data envelope', () async {
      ...
      expect(sample.display, equals('19.5'),
          reason: 'display falls back to value.toString() when absent from the payload');
  ...
  group('ReactClient error handling', () {
    ...
    test('timeout throws ReactUnavailable', () async {
      final MockClient mock = MockClient((http.Request req) async {
  ...
  group('ReactClient.heatmap()', () {
    test('GETs /api/v1/heatmaps/{id} with no query and decodes grid', () async {
      ...
      final MockClient mock = MockClient((http.Request req) async {
        expect(req.url.path, equals('/api/v1/heatmaps/entity-pressure-heatmap'));
      ...
      final HeatmapGrid grid =
          await client.heatmap('entity-pressure-heatmap');
    ...
    test('encodes world/center/radius query params', () async {
      ...
      final ReactClient client = ReactClient(cred, client: mock);
      await client.heatmap('id',
          world: 'world_nether', centerX: 3, centerZ: -2, radius: 8);
    });
