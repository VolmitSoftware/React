...
import 'package:reactor/service/relay_identity.dart';

void main() {
  const List<int> _spkiPrefix = <int>[
    0x30, 0x2a, 0x30, 0x05, 0x06, 0x03, 0x2b, 0x65,
    0x70, 0x03, 0x21, 0x00,
  ];

  final List<int> _rawKey = List<int>.generate(32, (int i) => i + 1);

  final List<int> _x509Bytes = <int>[..._spkiPrefix, ..._rawKey];

  String _buildExpectedFingerprint(List<int> x509Bytes) {
    final List<int> digest = sha256.convert(x509Bytes).bytes;
    return digest
        .sublist(0, 8)
  ...
  final String _expectedFingerprint = _buildExpectedFingerprint(_x509Bytes);

  final String _pubKeyBase64 = base64Url.encode(_x509Bytes);

  group('RelayIdentity.fingerprint', () {
    test('computes sha256-first-8-colon-hex over X509 bytes', () {
      final String fp = RelayIdentity.fingerprint(_pubKeyBase64);
      expect(fp, equals(_expectedFingerprint));
    });
    ...
    test('matches hardcoded vector for known input', () {
      ...
      final String expected = _buildExpectedFingerprint(known);
    ...
    test('returns false-safe empty string on bad base64 input', () {
      expect(() => RelayIdentity.fingerprint('!!!not-base64!!!'), returnsNormally);
    });
  });
