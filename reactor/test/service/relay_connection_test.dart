  ...
  group('RelayRpcMux', () {
    ...
    test(
      ...
      final Map<String, String> headers =
          (f.payload?['headers'] as Map).cast<String, String>();
      expect(headers['Authorization'], equals('Bearer t'));
      mux.onFrame(RelayFrame(
    ...
    test('matching data frame completes future with correct status and body',
        () async {
      final Future<RelayResponse> future = mux.request(
        method: 'GET',
        path: '/api/v1/metrics',
      ...
      expect(
          (resp.body['data'] as Map<String, dynamic>)['tps'], equals(20));
    });

    test('data frame with non-matching requestId leaves future pending',
        () async {
      bool completed = false;
      final Future<RelayResponse> future =
          mux.request(method: 'GET', path: '/test', headers: <String, String>{});
      ...
      mux.failAll(RelayResponse(
          status: 503, body: <String, dynamic>{'error': 'cleanup'}));
    });

    test('error frame without requestId fails all pending with status 503',
        () async {
      final Future<RelayResponse> f1 =
          mux.request(method: 'GET', path: '/a', headers: <String, String>{});
      final Future<RelayResponse> f2 =
          mux.request(method: 'POST', path: '/b', headers: <String, String>{});
      mux.onFrame(RelayFrame(
    ...
    test(
        ...
        () async {
      final Future<RelayResponse> f1 =
          mux.request(method: 'GET', path: '/x', headers: <String, String>{});
      final Future<RelayResponse> f2 =
          mux.request(method: 'GET', path: '/y', headers: <String, String>{});
