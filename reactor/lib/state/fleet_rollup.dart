...
class FleetRollup {
  ...
  static FleetRollup compute({
    ...
    for (final FleetServerLive srv in servers) {
      ...
      final int serverAlertCount =
          openAlerts.where((FleetAlert a) => a.serverId == srv.id).length;
      ...
      healthList.add(FleetServerHealth(
        id: srv.id,
        name: srv.name,
        state: srv.state,
        tps: tps,
    ...
    final double finalWorstTps =
        tpsCount > 0 ? (worstTps == double.infinity ? 0.0 : worstTps) : 0.0;
    ...
    final List<FleetServerHealth> needsAttention = healthList
        .where((FleetServerHealth s) =>
      ...
      ..sort((FleetServerHealth a, FleetServerHealth b) {
        final int hcmp =
            healthOrder(a.health).compareTo(healthOrder(b.health));
