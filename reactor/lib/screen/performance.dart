...
class PerformanceScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final SamplerSample? spikeRate = scope?.snapshot?.sampler('tick-spike-rate');
    ...
    final List<(String, List<double>)> tickSeries =
        <(String, List<double>)>[
      ('Tick Time', tickTime?.history ?? const <double>[]),
      if (p50 != null) ('p50', p50.history),
      if (p95 != null) ('p95', p95.history),
      if (p99 != null) ('p99', p99.history),
    ];

    final List<(String, List<double>)> spikeSeries =
        <(String, List<double>)>[
      ('Spike Rate', spikeRate?.history ?? const <double>[]),
    ];

    return ReactorPage(
      title: 'Performance',
      subtitle: 'Tick timing and load hotspots',
      children: <Widget>[
        sectionCard(
          label: 'Tick Duration',
        ...
        dom.div(
          ...
          <Widget>[
            StatTile(label: 'Top World MSPT', sample: worldMspt),
            StatTile(label: 'Top Chunk Cost', sample: chunkCost),
          ],
        ),
      ],
    );
