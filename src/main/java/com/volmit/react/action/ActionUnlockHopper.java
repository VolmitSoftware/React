			...
			if(i.getType().equals(Chunk.class))
			{
				...
				for(Object j : i.getPossibilities())
				{
					if(i.can(j))
					{
						...
						{
							completeAction();
							source.sendResponseSuccess(Lang.getString("action.unlock-hopper.unlocked-in") + F.f(completed.get()) + Lang.getString("action.unlock-hopper.chunk") + ((completed.get() > 1 || completed.get() == 0) ? "s" : "")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
						}
					}
				}
			}
