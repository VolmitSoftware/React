...
public class Catalyst15 extends CatalystPacketListener implements CatalystHost
{
	...
	{
		AdvancementHolder15 a = new AdvancementHolder15(UUID.randomUUID().toString());
		a.withToast(true);
		a.withDescription("?");
		a.withFrame(type);
		...
		addGlobalIncomingListener(new PacketHandler<Object>()
		{
			...
			public Object onPacket(Player player, Object packet)
			{
				if(packet instanceof PacketPlayInSettings)
				{
					...
					playerSettings.put(player, new PlayerSettings(new V(s).get("a"), new V(s).get("viewDistance"), ChatMode.values()[((EnumChatVisibility) new V(s).get("c")).ordinal()], new V(s).get("d"), new V(s).get("e"), new V(s).get("f").equals(EnumMainHand.RIGHT)));
				}
